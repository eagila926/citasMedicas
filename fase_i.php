...
<div class="wrapper">    
    ...
    <script>
        ...
        $(document).on("click", "#btn-continuar-item6", function (e) {
            e.preventDefault();
            const segundoItem = $("#plan-lista li:nth-child(6)").text().trim();
            ...
            if (archivo) {
                ...
                const contenidoFasei = document.querySelector(".primerafase").innerHTML;
                localStorage.setItem("html_primerafase", contenidoFasei);

                window.location.href = `${archivo}?cedula=${cedula}`;
            }
            else {
                alert("No se encontró una ruta para el ítem 6 del plan.");
            }
        });
